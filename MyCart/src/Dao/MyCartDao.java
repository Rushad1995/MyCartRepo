...
public class MyCartDao {
	...
	public static boolean isAdminValid(AuthPojo userDetails) {
		...
		try {
			...
			ResultSet rs = st.executeQuery("SELECT * FROM MyCart.Adminlogin WHERE AdminId " + "= '"
					+ userDetails.getAdminId() + "' AND AdminPass = '" + userDetails.getAdminPass() + "'");
			while (rs.next()) {
				validStatus = true;
			}
	...
	public static void addProd(MyCartPojo product) {
		String status = "Product not able to add in ProductList";
		try {
			Connection conn = ConnectionDB.getConnection();
			PreparedStatement ps = conn.prepareStatement("INSERT INTO ProductList VALUES(?,?,?,?)");
	...
	public static boolean delCartegory(String category) {

		try {
			Connection conn = ConnectionDB.getConnection();
			PreparedStatement ps = conn.prepareStatement("delete FROM ProductList WHERE prodCategory = ?");
			ps.setString(1, category);
			boolean status = ps.execute();
			if (status) {
				System.out.println("Category removed from cart");
			} else {
				System.out.println("plz select valid category from Cart");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static boolean isUserValid(AuthPojo2 userDetails) {
		...
		try {
			...
			ResultSet rs = st.executeQuery("SELECT * FROM MyCart.Userlogin WHERE UserId " + "= '"
					+ userDetails.getUserId() + "' AND Password = '" + userDetails.getPassword() + "'");
			while (rs.next()) {
				validStatus = true;
			}
	...
	public static MyCartPojo getProdById(int prodId) {
		...
		try {
			...
			while (rs.next()) {
				product = new MyCartPojo(rs.getInt("prodId"), rs.getString("prodName"), rs.getString("prodCategory"),
		...
		try {
			...
			for (MyCartPojo product : prodList) {
				...
				System.out.println(product.getProdId() + ", " + product.getProdName() + ", "
						+ product.getProdCategory() + ", " + product.getProdPrice());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static void billing(String Uname) {
		...
		try {
			...
			System.out.println("Total Bill Amount : " + TotalBilling);
...
public static List<MyBillPojo> readBill() {

	System.out.println("Total Customer Bills :");

	List<MyBillPojo> prodList = new ArrayList<MyBillPojo>();
	try {
		Connection conn = ConnectionDB.getConnection();
		PreparedStatement ps = conn.prepareStatement("SELECT * FROM billing_backup");
		ResultSet rs = ps.executeQuery();
		while (rs.next()) {

			MyBillPojo prod = new MyBillPojo(rs.getString("User"),
					rs.getString("billing_items"), rs.getDouble("toatl_bill"));
			prodList.add(prod);
		}
		System.out.println("User  Product Ids  Toatl bill");
		for (MyBillPojo product : prodList) {

			System.out.println("["+ product.getUser() + "] ["
					+ product.getBilling_items() + "] [" + product.getToatl_bill()+"]");
		}
	} catch (Exception e) {
		e.printStackTrace();
	}
